...
1. Fill in `/* TODO 1 */`: define the `JourneyHelpers` object with a `signUp` step that navigates to `/signup`, fills `signup-email` and `signup-password`, clicks `signup-submit`, and asserts the user lands on the verify-email route.

2. Fill in `/* TODO 2 */`: implement `verifyEmail` — navigate to `/verify-email?token=test-token-123` and assert `verify-email-status` is visible.

3. Fill in `/* TODO 3 */`: implement `createWorkspace` — navigate to `/onboarding/workspace`, fill `workspace-name`, submit, and assert redirect to `/dashboard`.

4. Fill in `/* TODO 4 */`: implement `createProject` — click `create-project-button`, fill `project-name-input`, confirm, and assert `project-card` is visible.

5. Fill in `/* TODO 5 */`: implement `createTask` — open the project, click `add-task-button`, fill `task-title-input`, and assert `task-card` is visible.

6. Fill in `/* TODO 6 */`: create `contextA` with `browser.newContext()` using `storageState` for `user-a.json` and `contextB` using `storageState` for `user-b.json`.

7. Fill in `/* TODO 7 */`: in `contextA`'s page, assign the task to user B by clicking `task-assignee-select` and choosing user B's name.

8. Fill in `/* TODO 8 */`: in `contextB`'s page, navigate to `/dashboard` and assert `task-assignee` contains user B's display name.

9. Fill in `/* TODO 9 */`: use `browser.newContext({ storageState: savedStatePath })` to restore the session without re-running the full signup flow.

10. Fill in `/* TODO 10 */`: assert the dashboard heading is visible, confirming the restored session is fully authenticated.

**Validation command:**
```bash
npx playwright test tests/module-33-user-journeys/exercise.spec.ts --reporter=list
```
